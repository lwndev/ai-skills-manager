...
4. Create `src/commands/scaffold-interactive.ts` with:
   ...
   - `detectConflictingFlags(options: CliScaffoldOptions): string[]` — returns list of template-content flag names that were set alongside `--interactive` (the flags: `template`, `context`, `agent`, `userInvocable`, `hooks`, `minimal`, `description`, `allowedTools`, `memory`, `model`, `argumentHint`)
   - `TEMPLATE_CONTENT_FLAGS` constant — the list of flag names that `--interactive` overrides (includes `userInvocable` even though there is no interactive prompt for it — `userInvocable` is implicitly determined by template type: `internal` sets it to `false`, all others default to `true`)
...
2. Implement template type selection prompt (FR-2) using `select` from `@inquirer/prompts`:
   - Choices: `basic`, `forked`, `with-hooks`, `internal`, `agent` with descriptions
   - Default: `basic`
3. Implement context type selection prompt (FR-3) using `select`:
   - Only shown when template is `basic` (other templates set context implicitly — `forked` and `agent` default to `fork`; `with-hooks` and `internal` default to `inherit`; these defaults are handled by the existing scaffold API, not by the interactive module)
...
5. Implement memory scope prompt (FR-5) using `select`:
   - For `agent` template: show as `Memory scope:` with default `project`, no skip option
   - For other templates: show as `Memory scope (optional, press Enter to skip):` with skip as default
   - Choices: `user`, `project`, `local` (plus `skip` for non-agent templates)
6. Implement model selection prompt (FR-6) using `select`:
   - For `agent` template: show as `Model:` with default `sonnet`, no skip option
   - For other templates: show as `Model (optional, press Enter to skip):` with skip as default
   - Choices: `sonnet`, `opus`, `haiku` (plus `skip` for non-agent templates)
7. Implement hooks prompt (FR-7) using `confirm`:
   - Only shown for `basic` and `forked` templates
   - Default: No
...
14. Write unit tests in `tests/unit/commands/scaffold-interactive.test.ts`:
    ...
    - Memory defaults to `project` for `agent` template, skip for others
    - Model defaults to `sonnet` for `agent` template, skip for others
    - Hooks prompt skipped for `with-hooks`, `internal`, and `agent` templates
...
4. Write unit tests for summary formatting:
   ...
   - All optional fields appear when set
...
- Existing validation functions in `src/commands/scaffold.ts` (`validateAgent()`, `validateMemoryScope()`, `validateModel()`, `validateArgumentHint()`)
...
- Template defaults for `agent` (memory → `project`, model → `sonnet`)
...
- Verify each template type (`basic`, `forked`, `with-hooks`, `internal`, `agent`) produces correct output
...
- FEAT-017 (Agent Template Type) — already complete; provides the `agent` template, `--memory`, `--model`, and `--argument-hint` flags
...
- Interactive prompts guide user through all 10 configuration steps with correct conditional logic
- `agent` template type is available and defaults memory to `project`, model to `sonnet`
...
- Warning displayed when `--interactive` overrides template-content flags
