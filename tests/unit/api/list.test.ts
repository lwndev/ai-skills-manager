...
`;
  ...
  describe('return type', () => {
    it('returns a ListResult object with skills array', async () => {
  ...
  describe('empty results', () => {
    ...
    it('returns empty array when directory does not exist', async () => {
      const { skills } = await list({ targetPath: path.join(tempDir, 'nonexistent') });
    ...
    it('returns empty array when target path is a file (not a directory)', async () => {
      ...
      const { skills } = await list({ targetPath: filePath });
  ...
  describe('skill metadata', () => {
    it('includes description from SKILL.md', async () => {
      ...
      expect(skills[0].description).toBe('A skill with a description');
    });
    ...
    it('includes absolute path to skill directory', async () => {
      ...
      expect(skills[0].path).toContain('path-test');
    });
  });
  ...
  describe('targetPath option', () => {
    it('searches only the specified path', async () => {
      ...
      const { skills } = await list({ targetPath: subDir });
  ...
  describe('error handling', () => {
    ...
    it('skips skills that cannot be read', async () => {
      ...
      expect(skills).toHaveLength(1);
      expect(skills[0].name).toBe('readable-skill');
    });
  });
