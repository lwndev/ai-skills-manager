...
describe('scaffold API integration', () => {
  ...
  describe('scaffold API with template options (FEAT-013 Phase 4)', () => {
    ...
    it('scaffold with license option passes validation', async () => {
      const result = await scaffold({
        name: 'api-license-skill',
        output: tempDir,
    ...
    it('scaffold with compatibility option passes validation', async () => {
      const result = await scaffold({
        name: 'api-compat-skill',
        output: tempDir,
        template: { compatibility: 'claude-code>=2.1' },
      });
    ...
    it('scaffold with metadata option passes validation', async () => {
      const result = await scaffold({
        name: 'api-metadata-skill',
        output: tempDir,
        template: { metadata: { author: 'team', version: '1.0' } },
      });
      ...
      expect(content).toContain('author: team');
    });
  ...
  describe('CLI scaffold with template options (FEAT-013 Phase 4)', () => {
    ...
    it('CLI scaffold --license creates valid skill', () => {
      execSync(
        `node "${cliPath}" scaffold cli-license-skill --output "${tempDir}" --license MIT --force`,
        { encoding: 'utf-8' }
      );

      const skillPath = path.join(tempDir, 'cli-license-skill');
      const content = fsSync.readFileSync(path.join(skillPath, 'SKILL.md'), 'utf-8');
      expect(content).toContain('license: MIT');
    });

    it('CLI scaffold --compatibility creates valid skill', () => {
      execSync(
        `node "${cliPath}" scaffold cli-compat-skill --output "${tempDir}" --compatibility "claude-code>=2.1" --force`,
        { encoding: 'utf-8' }
      );

      const skillPath = path.join(tempDir, 'cli-compat-skill');
      const content = fsSync.readFileSync(path.join(skillPath, 'SKILL.md'), 'utf-8');
      expect(content).toContain('compatibility: claude-code>=2.1');
    });

    it('CLI scaffold --metadata creates valid skill', () => {
      execSync(
        `node "${cliPath}" scaffold cli-metadata-skill --output "${tempDir}" --metadata author=team version=1.0 --force`,
        { encoding: 'utf-8' }
      );

      const skillPath = path.join(tempDir, 'cli-metadata-skill');
      const content = fsSync.readFileSync(path.join(skillPath, 'SKILL.md'), 'utf-8');
      expect(content).toContain('metadata:');
      expect(content).toContain('author: team');
    });
